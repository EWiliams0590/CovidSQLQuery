...
(
	SELECT date as Date, 
       CASE 
		   WHEN location <> 'New York State' THEN location
		   ELSE 'New York' 
	   END as State, 
	   people_fully_vaccinated as FullVax, 
	   people_vaccinated as Vax,
	   -- Get the null record with the same indicator as the first non-null record above
...
SELECT 
	Date, 
    State,
	-- Take the min from the previous CTE so that the appropriate value is filled in
